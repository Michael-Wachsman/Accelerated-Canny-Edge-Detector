...
void findEdges(uint8_t *image, uint8_t *out_image, int w, int h,
  ...
  dim3 num_blocks2((width + block_size.x - 1) / block_size.x,
                  (height + block_size.y - 1) / block_size.y);
  
  hysteresis_first<<<num_blocks2, block_size>>>(
    thresh_img, input_image, width, height,
    size); // reuse input_image as output cache
    
  for(int w =0; w < 4; w++){
  hysteresis<<<num_blocks2, block_size>>>(
      thresh_img, input_image, width, height,
      size); // reuse input_image as output cache
  }

  hysteresis_final<<<num_blocks2, block_size>>>(
      thresh_img, input_image, width, height,
      size); // reuse input_image as output cache
